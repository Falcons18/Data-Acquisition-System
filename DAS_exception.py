...
arduino = serial.Serial('COM16', 57600,timeout=0.1)     #Define serial ports
arduino1 = serial.Serial('COM15', 57600, timeout=0.1)
cv2.imshow('frame',cap)

time.sleep(3)   #wait for 3 seconds to receive arduino data

fh=open("height4.log","w")      #Open altitude log file
...
while True:
    try:
        ...
        if command == "h":
            ...
            if time.time() - t_end >= 1.0:      #Write into log file every 1 second
                fh.write(alt)
                t_end = time.time()
    ...
    # flag == 1 is checked to ensure that data is being received
        ...
        if (t == ord('d') or flag2 == 1) :                #Check for drop command or automatic payload drop
            flag2 = 1
            arduino.write("d")  
        elif t == ord('r'):                 #Check for reset command
            arduino.write("r")
        elif t == ord('q'):               #Check command for ending program
            break
        elif (t == ord('s') or t == ord('S')):
            arduino.write("s")
    except:
        pass
fh.close()
